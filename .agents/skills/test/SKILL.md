...
description: |
  ...
  Called by $dispatch with level argument. Ends by calling $checkpoint.
---

# $test — Execute One Test Layer

Called by $dispatch with a target level argument. Does one thing: execute all test items for that layer from the subtask verification plan.

## Execution Flow

### 1. Determine Target Level

- Has argument (e.g., `$test L2`) → use that level
...
3. After fixing code, record the fix in $checkpoint (don't just record test results — also record what code was changed)
...
### 6. Chain: Immediately Invoke $checkpoint

**After outputting the summary, immediately invoke `$checkpoint`. Do not wait for user instructions, do nothing else.**

## Key Constraints

- **Only run one layer**: Don't jump to the next layer — $dispatch decides the next step
- **Don't record results to file**: $checkpoint handles record-keeping
...
- **Chaining is mandatory**: Must invoke $checkpoint after completion, cannot skip or manually substitute
