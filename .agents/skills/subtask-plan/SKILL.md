...
description: |
  ...
  Call after $subtask-init. Makes subtask ready for code implementation.
---

# $subtask-plan — Complete Verification Plan (Gate 1.3 Step C+D+E)

Invoked after $subtask-init completes. Fills in upstream traceability table, L1-L4 verification plan, and solution self-check.
...
**Pre-checks**:
- Assumptions table "Result" column has empty rows → **FAIL**, prompt to complete $subtask-init first
...
→ Automatically invoking $dispatch
```

### 7. Chain: Immediately Invoke $dispatch

**After outputting the summary above, immediately invoke `$dispatch`. Do not wait for user instructions, do nothing else.**
