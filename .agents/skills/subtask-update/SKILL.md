...
description: |
  ...
  (A) Auto-called by $gate-check on FAIL — reads FAIL info, adds supplementary steps.
  (B) User calls with a finding/bug during testing — diagnoses, fixes, updates subtask.
  Both modes end by invoking $dispatch to re-enter the loop.
---

# $subtask-update — Supplement/Correct Subtask Plan

**Two trigger modes**:
- **Mode A**: Automatically called after `$gate-check` FAIL (no arguments)
- **Mode B**: User calls manually with a finding/bug description

Both modes ultimately return to the `$dispatch` loop.
...
Extract from conversation context the $gate-check FAIL output:
...
### C2. Chain: Immediately Invoke $dispatch

**After updating subtask, immediately invoke `$dispatch`. Do not wait for user instructions, do nothing else.**
...
- **Chaining is mandatory**: Must invoke $dispatch after completion
