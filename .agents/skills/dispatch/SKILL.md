...
description: |
  ...
  Called after $checkpoint, $gate-check, $subtask-update, $subtask-plan, or when resuming after context loss.
  When lost, call $dispatch.
---

# $dispatch — Execution Chain Router (Sole Routing Point)

Reads subtask.md checkbox state and routes to the next skill based on fixed priority.

**When to call**: Automatically called after $checkpoint, $gate-check, $subtask-update, $subtask-plan complete, or manually called after context loss.
...
1. **Has incomplete Phase?** → invoke `$code`
2. **All Phases complete, has untested layer?** → invoke `$test <level>` (pass the first incomplete level)
3. **All tests complete, no Gate result?** → invoke `$gate-check`
4. **Gate result = PASS?** → output completion report (**terminal point, do not invoke any further skill**)
5. **Gate result = FAIL?** → invoke `$subtask-update`
...
```
→ Invoking $code to implement Phase Y
```
or
```
→ Invoking $test L2
...
or
```
→ Invoking $subtask-update (Gate FAIL)
```
...
- **Loop-safe**: dispatch never calls itself — only calls $code, $test, $gate-check, or $subtask-update
