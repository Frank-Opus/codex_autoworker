...
description: |
  ...
  Called by $dispatch. Ends by calling $checkpoint.
---

# $code — Implement One Phase

Called by $dispatch. Does one thing: implement the first incomplete Phase from the subtask plan.
...
If subtask doesn't exist or plan is empty → **stop, prompt to complete $subtask-plan first**.
If all Phases are complete → **stop, prompt that $dispatch should route to $test**.
...
- Checking off boxes yourself ($checkpoint's responsibility)
- Running tests yourself ($test's responsibility)
...
### 4. Chain: Immediately Invoke $checkpoint
