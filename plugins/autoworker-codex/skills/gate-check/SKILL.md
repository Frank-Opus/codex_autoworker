...
description: |
  ...
  Writes PASS/FAIL result to subtask.md, then calls $dispatch for routing.
---

# $gate-check — Pre-Completion Self-Check (Gate 3)

**Trigger**: Called by $dispatch when all tests are complete. **Pure assessment skill — does not make routing decisions.**
...
- Test results section is empty → **FAIL**, prompt to complete tests and call `$checkpoint` first
...
### 6. Write Result and Invoke $dispatch
...
**FAIL loop limit**: If already consecutively FAIL 2 times (check "Progress Log" for `Gate result: FAIL` count), on the 3rd FAIL, do not invoke $subtask-update. Instead output a complete failure report to the user and let them decide next steps.
...
**Both cases always invoke `$dispatch`. No routing decisions — $dispatch reads Gate result and decides next step.**

### 7. Chain: Immediately Invoke $dispatch

**After outputting the result, immediately invoke `$dispatch`. Do not wait for user instructions, do nothing else.**
...
- **Gate result format is fixed**: Must be `Gate result: PASS` or `Gate result: FAIL` — $dispatch reads this exact format
- **Don't ask user "should I supplement?"**: Autonomously executable supplementary verification goes directly through $dispatch → $subtask-update loop
