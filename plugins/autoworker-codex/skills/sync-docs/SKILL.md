...
# $sync-docs — Sync Tracking Documents
...
- Project-level `AGENTS.md` — project rules and technical info
- `README.md` — project intro (for humans)

**Progressive disclosure documents**:
- `claude_docs/reference/*.md` — detailed documents referenced from AGENTS.md
- Subdirectory `AGENTS.md` — index files for each module
...
| `AGENTS.md` | Reusable patterns or lessons learned discovered | Add to appropriate section (don't duplicate existing content) |
...
| Subdirectory `AGENTS.md` | Changed code structure/files in a module | Update corresponding subdirectory file index |
...
**Dead link check**: Scan project AGENTS.md for all `claude_docs/reference/` references, confirm files exist:
- Referenced file exists → OK
- Referenced file doesn't exist → report dead link, suggest creating or fixing reference

**New reference identification**: If this conversation produced new lessons or technical documentation (> 20 lines) and AGENTS.md already exceeds 300 lines, suggest writing to `claude_docs/reference/<topic>.md` and adding a trigger heading in AGENTS.md, rather than appending directly to AGENTS.md.
...
**Flow**:
1. `Glob claude_docs/*/AGENTS.md` → get all subdirectory indexes
2. Read each subdirectory AGENTS.md (it's the semantic index listing files and purposes)
...
1. Determine sequence number: read `claude_docs/subtask/AGENTS.md` to find max sequence + 1
...
4. Update indexes:
   - `claude_docs/subtask/AGENTS.md` archive list + Phase grouping
   - `progress.md` add archive link
5. **Clear plan file**: Glob `.claude/plans/*.md`, find current project's plan file, clear contents (write empty string). Archiving = task done — stale plan would interfere with next $deep-plan.

### 6. Project AGENTS.md Health Check

Check project-level `AGENTS.md` size and structure (if exists):
...
Scan AGENTS.md sections, identify migratable content by these rules:
...
**Output**:
- Healthy → report "AGENTS.md: N lines, healthy"
...
- AGENTS.md: Added "YYY" lesson
- [archive] → claude_docs/subtask/40_xxx_20260227.md
- AGENTS.md health: N lines, healthy / getting large, suggest migrating X sections
```

## Important Notes

- **Don't fabricate content**: Only update based on what actually happened in the conversation — don't write uncertain things
- **AGENTS.md updates should be cautious**: Only add verified, reusable patterns — not one-time details
